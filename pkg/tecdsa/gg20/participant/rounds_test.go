...
// Test that P2PSend can be marshaled and unmarshaled correctly
// Also See TestMarshalResponseProof and TestMarshalJsonRange2Proof
func TestMarshalP2PSend(t *testing.T) {
	curve := btcec.S256()

	rpp := proof.ResponseProofParams{
		Curve: curve,
		Pk:    paillier.NewPubkey(tt.B10("10547615858184964306411562661985243216511226462060029912418561431293350513994423138528642902287903466486042818341097441827086959649889826199119664403007173")),
		DealerParams: &dealer.ProofParams{
	...
	assert.NoError(t, err)
	assert.NotNil(t, proofGamma)

	proofW, err := rpp.ProveWc()
	assert.NoError(t, err)
	assert.NotNil(t, proofW)
	...
	testJSON, err := json.Marshal(initSend)
	assert.NoError(t, err)
	assert.NotNil(t, testJSON)

	var actual P2PSend
	err = json.Unmarshal(testJSON, &actual)
	assert.NoError(t, err)

	assert.Equal(t, initSend, actual)
}
