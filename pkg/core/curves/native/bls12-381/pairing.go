...
// returns nil if no more pairs can be added
func (e *Engine) AddPair(g1 *PointG1, g2 *PointG2) *Engine {
	if len(e.pairs) > maxInputLength {
		return nil
	}
	p := newPair(g1, g2)
	if !e.isZero(p) {
...
func (e *Engine) preCompute(ellCoeffs *[68][3]fe2, twistPoint *PointG2) error {
	...
	for i := int(x.BitLen() - 2); i >= 0; i-- {
		if j > len(ellCoeffs) {
			return errors.New("precompute: j index greater than len of ellCoeffs")
		}
...
func (e *Engine) millerLoop(f *fe12) error {
	pairs := e.pairs
	ellCoeffs := make([][68][3]fe2, len(pairs))
	for i := 0; i < len(pairs); i++ {
		err := e.preCompute(&ellCoeffs[i], pairs[i].g2)
		if err != nil {
			return err
		}
	}
...
func (e *Engine) calculate() (*fe12, error) {
	f := e.fp12.one()
	if len(e.pairs) == 0 {
		return f, nil
	}
...
func (e *Engine) Check() (bool, error) {
	calc, err := e.calculate()
	if err != nil {
		return false, err
	}
	return calc.isOne(), nil
}
