...
func (fe *fe) cmp(fe2 *fe) int {
	...
	for i := 5; i > -1; i-- {
		//if fe[i] > fe2[i] {
		//	return 1
		gt |= int(((fe2[i]-fe[i])>>63)&1) & ^lt
		//} else if fe[i] < fe2[i] {
		//	return -1
		//}
		lt |= int(((fe[i]-fe2[i])>>63)&1) & ^gt
	}
	//return 0
	return gt - lt
}
...
// cmove sets self to y if v == 1 and does nothing if v == 0
// no branches
// Its behavior is undefined if v takes any other value.
func (fe *fe) cmove(v uint, y *fe) {
	b := -uint64(v)
	fe[0] ^= (fe[0] ^ y[0]) & b
	fe[1] ^= (fe[1] ^ y[1]) & b
	fe[2] ^= (fe[2] ^ y[2]) & b
...
func (e *fe6) equal(e2 *fe6) bool {
	res := e[0].equal(&e2[0])
	res = res && e[1].equal(&e2[1])
	res = res && e[2].equal(&e2[2])
	return res
}

func (e *fe6) cmove(v uint, y *fe6) {
	e[0].cmove(v, &y[0])
	e[1].cmove(v, &y[1])
	e[2].cmove(v, &y[2])
}
...
func (e *fe12) cmove(v uint, y *fe12) {
	e[0].cmove(v, &y[0])
	e[1].cmove(v, &y[1])
}
