...
func (g *G2) FromCompressed(compressed []byte) (*PointG2, error) {
	if len(compressed) != 96 {
		return nil, errors.New("input string should be exactly 96 bytes")
	}
	var in [96]byte
	copy(in[:], compressed[:])
	if in[0]&(1<<7) == 0 {
...
func (g *G2) toBytesJacobi(p *PointG2) []byte {
	out := make([]byte, 288)
	copy(out[:96], g.f.toBytes(&p[0]))    // x
	copy(out[96:192], g.f.toBytes(&p[1])) // y
	copy(out[192:], g.f.toBytes(&p[2]))   // z
	return out
}

func (g *G2) fromBytesJacobi(in []byte) (*PointG2, error) {
	if len(in) < 288 {
	...
	p1, err := g.f.fromBytes(in[96:192])
	if err != nil {
		return nil, err
	}
	p2, err := g.f.fromBytes(in[192:])
	if err != nil {
		return nil, err
	}
	p := &PointG2{*p0, *p1, *p2}
...
func (g *G2) MultiExp(r *PointG2, points []*PointG2, powers []*big.Int) (*PointG2, error) {
	...
	numBits := uint32(g.Q().BitLen())
	windows := make([]*PointG2, (numBits+c-1)/c)
	acc, sum := g.New(), g.New()
	mask := (uint64(1) << c) - 1
	j := 0
	var cur uint32
